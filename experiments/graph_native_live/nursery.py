...
def tokenize_surface_forms(
    ...
    result = _tokenize_payload(model, codec, forms)
    return [
        (
...
def tokenize_lexical_sequences(
    model: Path,
    codec: Path,
    forms: Sequence[str],
) -> list[
    ...
    """Return whole-utterance geometry plus one exact row per model token."""
    result = _tokenize_payload(model, codec, forms)
    sequences = []
    for item in result["items"]:
        token_ids = tuple(int(token) for token in item["token_ids"])
        embedding = tuple(float(value) for value in item["embedding"])
        token_embeddings = tuple(
            tuple(float(value) for value in row)
            for row in item.get("token_embeddings", ())
        )
        if len(token_embeddings) != len(token_ids):
            raise RuntimeError(
                "lexeme codec must expose one token embedding per sensed token; rebuild it"
            )
        if any(len(row) != int(result["dimension"]) for row in token_embeddings):
            raise RuntimeError("lexeme codec returned malformed token geometry")
        sequences.append((token_ids, embedding, token_embeddings))
    return sequences
