    ...
    try {
        ...
        if (mode == "tokenize") {
            ...
            for (int index = 3; index < argc; ++index) {
                ...
                std::cout << "],\"token_embeddings\":[";
                for (size_t token_index = 0; token_index < tokens.size(); ++token_index) {
                    if (token_index) std::cout << ',';
                    const auto token_embedding = lexical_embedding(
                        model.ptr,
                        std::vector<llama_token>{tokens[token_index]},
                        dimension);
                    std::cout << '[';
                    for (size_t embedding_index = 0;
                         embedding_index < token_embedding.size();
                         ++embedding_index) {
                        if (embedding_index) std::cout << ',';
                        std::cout << token_embedding[embedding_index];
                    }
                    std::cout << ']';
                }
                std::cout << "]}";
