...
class NativeMassEmbedder:
    ...
    def __init__(self, model: Path, codec: Path):
        self.model = model
        self.codec = codec
        # Populated only by the offline compiler with already embedded nursery
        # material. Live sensing never appends conversation text here.
        self.cache: dict[str, tuple[float, ...]] = {}
        self._current_digest: str | None = None
        self._current_embedding: tuple[float, ...] | None = None
        self._current_lexical_rows: tuple[tuple[float, ...], ...] | None = None
        self.bootstrap = True

    def _sense_native(self, text: str) -> None:
        digest = hashlib.sha256(text.encode("utf-8")).hexdigest()
        if (
            digest == self._current_digest
            and self._current_embedding is not None
            and self._current_lexical_rows is not None
        ):
            return
        _, vector, lexical_rows = nursery.tokenize_lexical_sequences(
            self.model, self.codec, (text,)
        )[0]
        self._current_digest = digest
        self._current_embedding = vector
        self._current_lexical_rows = lexical_rows
    ...
    def embed(self, text: str) -> list[float]:
        ...
        self._sense_native(text)
        if self._current_embedding is None:
            raise RuntimeError("native lexical sensor produced no utterance geometry")
        return list(self._current_embedding)

    def lexical_sequence(self, text: str) -> tuple[tuple[float, ...], ...]:
        if self.bootstrap:
            raise RuntimeError("lexical sensing is unavailable during opaque gestation")
        self._sense_native(text)
        if self._current_lexical_rows is None:
            raise RuntimeError("native lexical sensor produced no token geometry")
        rows = self._current_lexical_rows
        self._current_digest = None
        self._current_embedding = None
        self._current_lexical_rows = None
        return rows
