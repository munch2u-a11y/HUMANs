...
def test_native_embedder_reuses_one_call_for_utterance_and_token_rows(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls = []
    mean = tuple([0.25] * 1024)
    token_rows = (tuple([0.5] * 1024), tuple([-0.5] * 1024))

    def fake_sequence(_model, _codec, forms):
        calls.append(tuple(forms))
        return [((10, 11), mean, token_rows)]

    monkeypatch.setattr(
        GESTATION.nursery, "tokenize_lexical_sequences", fake_sequence
    )
    embedder = GESTATION.NativeMassEmbedder(Path("model"), Path("codec"))
    embedder.bootstrap = False

    assert embedder.embed("one current signal") == list(mean)
    assert embedder.lexical_sequence("one current signal") == token_rows
    assert calls == [("one current signal",)]
    assert embedder.cache == {}
    assert embedder._current_digest is None
    assert embedder._current_embedding is None
    assert embedder._current_lexical_rows is None
