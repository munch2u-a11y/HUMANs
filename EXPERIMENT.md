# Six-Lane Causal Membrane Subsystem

This document records the foundational runtime experiment in which six
independent causal lanes share one persistent Habitus graph without sharing one
...
```bash
python -m pip install -e '.[test,cortex]'
python -m pytest -q -ra
python -m habitus_ai.demo
```

Tests requiring a local Qwen GGUF model and compiled llama.cpp helpers skip when
those optional artifacts are unavailable. Generated binaries, model files,
SQLite minds, and run receipts are intentionally not committed.
...
This subsystem demonstrates deterministic modality isolation, independent lane
...
[`WHITEPAPER.md`](WHITEPAPER.md); the integrated release boundary is summarized
