...
from .recurrent import RecurrentField
...
class BaseAgenticMemoryRAG:
    ...
    def __init__(
        ...
        *,
        ...
        self.recurrent = RecurrentField(self.store, self.graph)
    ...
    def begin_pulse(self) -> tuple[int, str]:
        """Reserve one logical SELF transition for a complete sensory frame.

        Existing one-off calls still reserve their own pulse. Continuous
        runtimes reserve once and pass that pulse through every admitted input
        and externalized output belonging to the same transition.
        """
        return self._next_pulse()

    def _use_pulse(self, pulse_number: int | None) -> tuple[int, str]:
        if pulse_number is None:
            return self._next_pulse()
        resolved = int(pulse_number)
        if resolved < 1 or resolved != self.pulse:
            raise ValueError(
                "shared pulse_number must equal the currently reserved mind pulse"
            )
        return resolved, f"pulse:{resolved}"
    ...
    def remember(
        ...
        *,
        ...
        pulse_number, _ = self._use_pulse(pulse_number)
    ...
    def record_cycle_return(
        ...
        *,
        ...
        pulse_number: int | None = None,
        defer_recurrent_outcome: bool = False,
    ) -> CycleReturnResult:
        ...
        outcome = self.record_outcome(
            ...
            update_recurrent=not defer_recurrent_outcome,
        )
    ...
    def classify_output(
        ...
        *,
        ...
        pulse_number, pulse_id = self._use_pulse(pulse_number)
        if private:
    ...
    def record_outcome(
        ...
        *,
        ...
        if update_recurrent:
            self.recurrent.observe_outcome(
                edge_ids,
                stability_delta=stability_delta,
                verified=verified,
                pulse=self.pulse,
