...
@dataclass(frozen=True)
class SocialSourceState:
    """Learned relational preference for one opaque communication source."""
...
@dataclass(frozen=True)
class NodeDynamics:
    """Persistent recurrent state and open dynamics for one graph node.

    ``activation`` is short-lived working state. ``pressure`` is an unmet
    endogenous tendency that can accumulate without a new external message.
    The remaining scalar parameters are inspectable dynamics, not prompt text
    or a hidden policy table.
    """
    ...
    last_pulse: int = 0
    last_selected_pulse: int | None = None


@dataclass(frozen=True)
class DesireActivation:
    """One desire node's current auditable contribution to behavior."""
...
class RecurrentSnapshot:
    """A fixed-size logical state boundary; it contains no transcript text."""

    pulse: int
    node_states: tuple[NodeDynamics, ...]
    desires: tuple[DesireActivation, ...]
...
class DevelopmentalGrowth:
    """Bounded graph growth caused by one sensory experience or co-settled frame.

    These IDs are an audit receipt, not a semantic description.  Every listed
    node carries only numeric geometry and graph position; words remain on the
    separate language membrane.
    """

    record_ids: tuple[str, ...]
    input_trunks: tuple[InputTrunk, ...]
    feature_node_ids: tuple[str, ...]
    candidate_pattern_node_ids: tuple[str, ...]
    promoted_pattern_node_ids: tuple[str, ...]
    active_context_node_ids: tuple[str, ...]
    edge_ids: tuple[str, ...]
    cross_trunk: bool = False
