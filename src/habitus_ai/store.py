...
class MindStore:
    ...
    def transaction(self) -> Iterator[sqlite3.Connection]:
        """Hold one explicit transaction across cooperating store methods.

        Only methods using ``_write`` participate in a surrounding transaction;
        ordinary callers retain the store's historical per-method commits. The
        self-pulse kernel uses this narrow boundary to commit recurrent state,
        its pulse receipt, and output authorizations as one durable transition.
        """
        with self._lock:
        ...
        with self.connection:
            self.connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS social_source_state (
                    node_id TEXT PRIMARY KEY REFERENCES concepts(concept_id),
                    preference_mean REAL NOT NULL DEFAULT 0.0,
                    preference_weight REAL NOT NULL DEFAULT 0.0,
                    observation_count INTEGER NOT NULL DEFAULT 0,
                    last_pulse INTEGER NOT NULL
                ...
                CREATE TABLE IF NOT EXISTS node_dynamics (
                    ...
                    baseline_growth REAL NOT NULL DEFAULT 0.0,
                    persistence REAL NOT NULL DEFAULT 0.65,
                    satisfaction_gain REAL NOT NULL DEFAULT 0.70,
                    frustration_gain REAL NOT NULL DEFAULT 0.45,
                    expression_threshold REAL NOT NULL DEFAULT 1.0,
                    last_pulse INTEGER NOT NULL DEFAULT 0,
                    last_selected_pulse INTEGER
                );

                CREATE TABLE IF NOT EXISTS recurrent_pulses (
                    pulse_id TEXT PRIMARY KEY,
                    pulse INTEGER NOT NULL,
                    excitation_json TEXT NOT NULL DEFAULT '{}',
                    pressure_delta_json TEXT NOT NULL DEFAULT '{}',
                    dominant_desire_id TEXT,
                    should_express INTEGER NOT NULL,
                ...
                CREATE INDEX IF NOT EXISTS idx_node_dynamics_pressure
                    ON node_dynamics(pressure DESC, activation DESC);
                CREATE INDEX IF NOT EXISTS idx_recurrent_pulses_ordinal
                    ON recurrent_pulses(pulse);
                """
            )
    ...
    def has_concept(self, concept_id: str) -> bool:
        return (
            self.connection.execute(
                "SELECT 1 FROM concepts WHERE concept_id = ?", (str(concept_id),)
            ).fetchone()
            is not None
        )

    def concept_ids_by_kind(self, kinds: Iterable[str]) -> set[str]:
        ...
        return {
            str(row["concept_id"])
            for row in self.connection.execute(
                f"SELECT concept_id FROM concepts WHERE kind IN ({placeholders})",
                selected,
            ).fetchall()
        }
    ...
    def set_concept_kind(self, concept_id: str, kind: str) -> None:
        with self._write() as connection:
            connection.execute(
                "UPDATE concepts SET kind = ? WHERE concept_id = ?",
                (str(kind), str(concept_id)),
            )

    def vault_record_count(self, vault_id: str) -> int:
        row = self.connection.execute(
            "SELECT COUNT(DISTINCT record_id) AS total FROM vault_membership WHERE vault_id = ?",
            (str(vault_id),),
        ).fetchone()
    ...
    def edge_evidence_record_ids(
        ...
        *,
        ...
        if relation is None:
            rows = self.connection.execute(
                """
                SELECT record_id FROM edge_evidence
                WHERE edge_id = ? ORDER BY record_id
                """,
                (edge_id,),
            ).fetchall()
        else:
            rows = self.connection.execute(
                """
                SELECT record_id FROM edge_evidence
                WHERE edge_id = ? AND relation = ? ORDER BY record_id
                """,
                (edge_id, relation),
            ).fetchall()
    ...
    def update_social_source_state(
        ...
        *,
        ...
        """Update a source-specific weighted preference without storing its name."""
        bounded_preference = max(-1.0, min(1.0, float(preference)))
        bounded_confidence = max(0.0, min(1.0, float(confidence)))
        existing = self.connection.execute(
            "SELECT * FROM social_source_state WHERE node_id = ?",
            (str(node_id),),
        ).fetchone()
        old_mean = float(existing["preference_mean"]) if existing else 0.0
        old_weight = float(existing["preference_weight"]) if existing else 0.0
        ...
        total_weight = old_weight + bounded_confidence
        mean = (
            (old_mean * old_weight + bounded_preference * bounded_confidence)
            / total_weight
            if total_weight > 0.0
            else old_mean
        )
        with self.connection:
            self.connection.execute(
                """
                INSERT INTO social_source_state(
                    node_id, preference_mean, preference_weight,
                    observation_count, last_pulse
                ) VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(node_id) DO UPDATE SET
                    ...
                    last_pulse=excluded.last_pulse
                """,
                (str(node_id), mean, total_weight, old_count + 1, int(pulse)),
            )
        return SocialSourceState(
            node_id=str(node_id),
            preference_mean=mean,
            preference_weight=total_weight,
            observation_count=old_count + 1,
    ...
    def get_social_source_state(self, node_id: str) -> SocialSourceState | None:
        row = self.connection.execute(
            "SELECT * FROM social_source_state WHERE node_id = ?",
            (str(node_id),),
        ).fetchone()
    ...
    def list_social_source_states(self) -> list[SocialSourceState]:
        rows = self.connection.execute(
            "SELECT * FROM social_source_state ORDER BY node_id"
        ).fetchall()
        return [
            SocialSourceState(
                node_id=str(row["node_id"]),
    ...
    # ------------------------------------------------------- recurrent node field

    @staticmethod
    def _node_dynamics_from_row(row: sqlite3.Row) -> NodeDynamics:
        return NodeDynamics(
            node_id=str(row["node_id"]),
            activation=float(row["activation"]),
            ...
            baseline_growth=float(row["baseline_growth"]),
            persistence=float(row["persistence"]),
            satisfaction_gain=float(row["satisfaction_gain"]),
            frustration_gain=float(row["frustration_gain"]),
            expression_threshold=float(row["expression_threshold"]),
            last_pulse=int(row["last_pulse"]),
            last_selected_pulse=(
                int(row["last_selected_pulse"])
                if row["last_selected_pulse"] is not None
                else None
            ),
        )

    def put_node_dynamics(self, state: NodeDynamics) -> NodeDynamics:
        if self.get_concept(state.node_id) is None:
            raise KeyError(f"unknown recurrent node: {state.node_id}")
        bounded = NodeDynamics(
            node_id=state.node_id,
            activation=max(0.0, min(1.0, float(state.activation))),
            pressure=max(0.0, min(1.0, float(state.pressure))),
            valence=max(-1.0, min(1.0, float(state.valence))),
            momentum=max(-1.0, min(1.0, float(state.momentum))),
            baseline_growth=max(0.0, min(1.0, float(state.baseline_growth))),
            persistence=max(0.0, min(1.0, float(state.persistence))),
            satisfaction_gain=max(0.0, min(1.0, float(state.satisfaction_gain))),
            frustration_gain=max(0.0, min(1.0, float(state.frustration_gain))),
            expression_threshold=max(0.0, min(2.0, float(state.expression_threshold))),
            last_pulse=max(0, int(state.last_pulse)),
            last_selected_pulse=(
                max(0, int(state.last_selected_pulse))
                if state.last_selected_pulse is not None
                else None
            ),
        )
        with self._write():
            self.connection.execute(
                """
                INSERT INTO node_dynamics(
                    node_id,activation,pressure,valence,momentum,
                    baseline_growth,persistence,satisfaction_gain,
                    frustration_gain,expression_threshold,last_pulse,
                    last_selected_pulse
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
                ON CONFLICT(node_id) DO UPDATE SET
                    ...
                    frustration_gain=excluded.frustration_gain,
                    expression_threshold=excluded.expression_threshold,
                    last_pulse=excluded.last_pulse,
                    last_selected_pulse=excluded.last_selected_pulse
                """,
                (
                    bounded.node_id,
                    bounded.activation,
                    ...
                    bounded.valence,
                    bounded.momentum,
                    bounded.baseline_growth,
                    bounded.persistence,
                    bounded.satisfaction_gain,
                    bounded.frustration_gain,
                    bounded.expression_threshold,
                    bounded.last_pulse,
                    bounded.last_selected_pulse,
                ),
            )
        return bounded

    def get_node_dynamics(self, node_id: str) -> NodeDynamics | None:
        row = self.connection.execute(
            "SELECT * FROM node_dynamics WHERE node_id = ?", (str(node_id),)
        ).fetchone()
        return self._node_dynamics_from_row(row) if row is not None else None

    def list_node_dynamics(self) -> list[NodeDynamics]:
        rows = self.connection.execute(
            "SELECT * FROM node_dynamics ORDER BY node_id"
        ).fetchall()
        return [self._node_dynamics_from_row(row) for row in rows]

    def save_recurrent_pulse(
        self,
        *,
        pulse_id: str,
        pulse: int,
        excitation: Mapping[str, float],
        pressure_delta: Mapping[str, float],
        dominant_desire_id: str | None,
        should_express: bool,
        state_sha256: str,
    ) -> None:
        with self._write():
            self.connection.execute(
                """
                INSERT INTO recurrent_pulses(
                    pulse_id,pulse,excitation_json,pressure_delta_json,
                    dominant_desire_id,should_express,state_sha256
                ) VALUES (?,?,?,?,?,?,?)
                """,
                (
                    str(pulse_id),
                    int(pulse),
                    _json(dict(excitation)),
                    _json(dict(pressure_delta)),
                    dominant_desire_id,
                    int(bool(should_express)),
                    str(state_sha256),
                ),
            )
