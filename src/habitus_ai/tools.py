...
PulseAdvancer = Callable[[Sequence[str]], CognitiveCycleReceipt | None]
...
class ToolRegistry:
    ...
    def __init__(
        ...
        *,
        ...
        pulse_advancer: PulseAdvancer | None = None,
    ) -> None:
        if pulse_advancer is not None and pulse_kernel is None:
            raise ValueError("a pulse advancer requires a pulse kernel")
        self.mind = mind
        self.pulse_kernel = pulse_kernel
        self.pulse_advancer = pulse_advancer
    ...
    def register_tool(self, tool: ToolDefinition) -> None:
        ...
        self.mind.add_concept(
            ...
            input_trunks=(
                OUTPUT_RETURN_LANES[tool.trunk]
                if self.pulse_kernel is not None
                else InputTrunk.SEE,
            ),
    ...
    def execute(
        ...
        *,
        ...
        affordance: OutputAffordance | None = None,
    ) -> ToolReceipt:
        ...
        *,
        ...
        if self.pulse_kernel is not None:
            if decision is not None:
                raise ValueError(
                    "kernel-backed tool execution accepts an affordance, not a decision"
                )
            if (
                affordance is None
                or affordance.trunk != tool.trunk
                or affordance.node_id != tool.tool_id
            ):
                raise ValueError(
                    "tool execution requires its selected self-pulse affordance"
                )
            cycle = self.pulse_kernel.actualize_output(
                affordance,
                invocation,
            ...
            if affordance is not None:
                raise ValueError("an affordance requires a pulse-backed registry")
            if decision is None:
            ...
            if (
                ...
                raise ValueError(
                    "the supplied output decision does not terminate at this ability"
                )
    ...
    def _finish_execution(
        ...
        *,
        ...
        if self.pulse_kernel is None:
            returned = self.mind.record_cycle_return(
                cycle.cycle_id,
                payload_text,
                input_trunk=return_lane,
                status=status,
            ...
            bucket = self.pulse_kernel.enqueue_cycle_return(
                cycle.cycle_id,
                payload_text,
                lane=return_lane,
                status=status,
                ...
                metadata=metadata,
                embedding=sensory_embedding,
                item_id=f"tool-return:{receipt_id}",
            )
            pulse = (
                self.pulse_advancer((bucket.item_id,))
                if self.pulse_advancer is not None
                else self.pulse_kernel.advance_cycle(
                    cycle_id=f"tool-return-pulse:{receipt_id}",
                    item_ids=(bucket.item_id,),
                )
            )
            if pulse is None:
                raise RuntimeError("tool return did not produce a self pulse")
            return_record_id = pulse.sensory.receipts[0].record_id
            outcome_id = None
            for row in self.mind.store.connection.execute(
                ...
                if outcome.get("receipt_id") == return_record_id:
                    outcome_id = str(row["outcome_id"])
                    break
            if outcome_id is None:
                raise RuntimeError("tool return has no persisted outcome receipt")
        return ToolReceipt(
            receipt_id=receipt_id,
            tool_id=tool.tool_id,
            trunk=tool.trunk,
